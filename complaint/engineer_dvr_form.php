<?php $this->load->view('header');?>
        ...
        <div class="col-md-12"> 
          ...
          <div class="portlet box grey-cascade">
           ...
           <form class="form-horizontal" method="post" enctype="multipart/form-data" action="<?php echo base_url();?>complaint/insert_dvr_engineer">
            ...
            <div class="form-body">
                ...
                <div class="col-md-12"> 
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-8">
								<?php 
										$useroffice	=	'0';
										$quw="SELECT fk_office_id from user where id =  '".$this->session->userdata('userid')."'";
                                        $ghw=$this->db->query($quw);
                                        $rtw=$ghw->result_array();
										$useroffice	= 'officeoption_'.$rtw[0]['fk_office_id'];
								?>
                                <input type="hidden" id="useroffice" value="<?php echo $useroffice; ?>" >
                                    <select class="form-control  " name="customer[0]" id="customer0"  onchange="show_cities(this.value,0)">
                                      ...
                                      <?php $quw="SELECT * from user where id =  '".$this->session->userdata('userid')."'";
                                        ...
                                        if($rtw[0]['fk_office_id']=='1')
										{
											$qu="SELECT * from tbl_clients where fk_office_id IN('".$rtw[0]['fk_office_id']."','5') AND (delete_status='0' OR pk_client_id<0)";
										}
										else
										{
											$qu="SELECT * from tbl_clients where fk_office_id =  '".$rtw[0]['fk_office_id']."' AND (delete_status='0' OR pk_client_id<0)";
										}
                                        $gh=$this->db->query($qu);
                                        $rt=$gh->result_array();
                                        foreach($rt as $value)
					...
					<!--	<div class="form-group" style="display:none;" id="outstation_check">
                                <label class="col-md-3 control-label">Outstation</label>
                                <div class="col-md-4">
                                  <input type="checkbox" name="outstation" />
                                </div>
                              </div> -->
			  ...
			  <script type="text/javascript">
				...
				function fill_business_dec_and_timeelapsed(business_id,rowid)
				{
				  ...
				  $.ajax({
					url: "<?php echo base_url();?>complaint/business_dec_ajax_against_tsnumber",
					type: 'POST',
					data: formdata,
					success: function(msg){
						$(".business_description"+rowid).val(msg);
					...
					$.ajax({
					url: "<?php echo base_url();?>complaint/business_time_elapsed_ajax_against_tsnumber",
					type: 'POST',
					data: formdata,
					success: function(msg){
						$(".time_elaped"+rowid).val(msg);
