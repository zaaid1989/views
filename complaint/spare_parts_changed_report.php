...
?>
                    ...
                    <!-- BEGIN PAGE HEADER-->
                    <h3 class="page-title">
                    Spare Parts Changed Report <small><?php //echo $average_visits_per_day; ?></small>
                    </h3>
                    <div class="page-bar">
		...
		<div class="portlet light bg-inverse">
						<div class="portlet-title">
							<div class="caption">
								...
								Spare Parts Changed Report </span>
								<span class="caption-helper">All Parts Against SPRF Approved</span>
							</div>
						</div>
						<div class="portlet-body">
						        <div class="row">
                            	<form method="get" action="<?php echo base_url();?>complaint/spare_parts_changed_report">
								<?php /*
                                <div class="col-md-4">
          ...
          <div class="portlet light bg-inverse<?php //echo $portlet_color[$k]; ?>">
            <div class="portlet-title">
              <div class="caption"> <i class="icon-pie-chart font-green"></i><span class="caption-subject bold font-green ">
						Spare Parts Changed - <?php echo $start_date_f.' to '.$end_date_f ?> </span></div>
              <div class="tools"> 
                ...
                <table class="table  table-hover " id="sample_225">
					...
					<?php  // Current Pending Calls
						...
						foreach ($result AS $pmc) {
							...
							echo '<tr> ';
							echo '<td>'.date('d-M-Y',strtotime($pmc['stock_date'])).'</td>';
							...
							echo '<td>'.$pmc['unit_price'].'</td>';
							echo '<td>'.$pmc['first_name'].'</td>';
							echo '<td>'.$pmc['part_source'].'</td>';
							$rq 	= $this->db->query("SELECT * FROM tbl_stock WHERE fk_stock_id='".$pmc['pk_stock_id']."'");
							$rb		= $rq->result_array();
							if (sizeof($rb)>0) {
								$total_rb = 0;
								foreach ($rb AS $received_part) {
									if ($received_part['stock']>1) $total_rb += $received_part['stock'];
									else $total_rb += 1;
								}
								echo '<td>Yes ('.$total_rb.')</td>';
							}
							else echo '<td>No</td>';
							echo '<td>';
