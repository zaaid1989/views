...
?>
      ...
      <h3 class="page-title">  Spare Part   <small>Update Order </small> </h3>
      ...
      <div class="page-bar">
        ...
        <ul class="page-breadcrumb">
          ...
          <li> <a href="<?php echo site_url();?>sys/parts_ordered">Parts Ordered</a> <i class="fa fa-angle-right"></i> </li>
        ...
        <div class="col-md-12">
                ...
                <div class="portlet box green-seagreen">
                  ...
                  <div class="portlet-title">
                    ...
                    <div class="caption"> <i class="fa fa-gift"></i>Update Order</div>
                  ...
                  <?php
                   ...
                   <!-- BEGIN FORM-->
            <form action="<?php echo base_url();?>sys/spare_part_update_order_insert" class="form-horizontal" method="post"  enctype="multipart/form-data">
                <div class="form-body">
				<div class="">
				<!--
	  <h3>
	  ...
	  -->
				...
				<div class="form-group">
                  <div class="col-md-4  col-md-offset-3 office_table">
					...
					$comments_for_update = "";
					$onumber_for_update = "";
					$parts_query="SELECT tbl_parts.`pk_part_id`, tbl_parts.`fk_instrument_id`, tbl_parts.`fk_vendor_id`, tbl_parts.`fk_product_id`, tbl_parts.`part_name`, tbl_parts.`part_number`, tbl_parts.`order_status`, tbl_parts.`description`, tbl_parts.`stock_quantity`, tbl_parts.`minimum_quantity`, tbl_parts.`unit_price`, tbl_parts.`comments`, tbl_parts.`image`, vendor_name, product_name,
					tbl_orders.`comments` AS 'commentss',tbl_orders.`order_quantity`,tbl_orders.`fk_part_id`,tbl_orders.`date`,tbl_orders.`order_number`
					FROM tbl_orders
								INNER JOIN tbl_parts ON tbl_parts.pk_part_id = tbl_orders.fk_part_id 
								INNER JOIN tbl_vendors ON tbl_parts.fk_vendor_id = tbl_vendors.pk_vendor_id 
								INNER JOIN tbl_products ON tbl_parts.fk_product_id = tbl_products.pk_product_id WHERE pk_order_id='".$this->uri->segment(3)."'";
								...
								{} 
									...
									$date_for_update = $rt22[0]['date'];
					$quantity_for_update = $rt22[0]['order_quantity'];
					$comments_for_update = $rt22[0]['commentss'];
					$onumber_for_update = $rt22[0]['order_number'];
								  foreach ($rt22 as $get_users_list) {
									  // for finding total stock
									  $part_idd = $get_users_list['fk_part_id'];
										$stock_total=0;
										$stock_demand=0;
										$stock_demand_aggregate=0;
										$stock = $this->db->query("select * from tbl_stock where  fk_part_id='".$get_users_list['fk_part_id']."'");
										  if($stock->num_rows()>0)
										  {
										  ...
										  $qd = $this->db->query("SELECT fk_part_id, SUM(quantity) AS demand FROM `tbl_sprf` WHERE status=0 AND fk_part_id = '".$get_users_list['fk_part_id']."' GROUP BY fk_part_id");
                                          $rd = $qd->result_array();
										  if (sizeof($rd)>0)
										  ...
										  $qd = $this->db->query("SELECT `tbl_sprf`.fk_complaint_id,`tbl_sprf`.quantity, `tbl_clients`.client_name,`tbl_complaints`.ts_number
											...
											WHERE `tbl_sprf`.status=0 AND `tbl_sprf`.fk_part_id = '".$get_users_list['fk_part_id']."'");
                                          $rd = $qd->result_array();
										  if (sizeof($rd)>0)
												foreach ($rd AS $dd) {
										...
										$qd = $this->db->query("SELECT * from tbl_orders
											WHERE status=1 AND fk_part_id = '".$get_users_list['fk_part_id']."' ORDER BY date ASC");
                                          $rd = $qd->result_array();
										  if (sizeof($rd)>0)
												foreach ($rd AS $dd) {
...
/*  */                                     } 
                    ...
                    <div class="col-md-4">
                        <select class="form-control" id="vendor_name" name="vendor_name" onchange="part_number_select(this.value)">
							...
							<?php 
								  ...
								  {
									  $query1 = $this->db->query("select * from tbl_parts where pk_part_id= '".$part_idd."'");
								  ...
								  {
							?>
                            		<option value="<?php echo $vendor['pk_vendor_id']?>" <?php if($part_idd!=''){if($vendor['pk_vendor_id']==$result1[0]['fk_vendor_id']){ echo ' selected="selected"';}}?>>
				 ...
				 function select_description(part_id)
				 {
					...
					$.ajax({
					url: "<?php echo base_url();?>sys/office_table_ajax",
					type: 'POST',
					data: formdata2,
					success: function(msg){
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-4">
                       <textarea name="part_description" class="input-xlarge" id="part_description" rows="5" required><?php if($part_idd!=''){ echo urldecode($result1[0]['description']);}?></textarea>
                    </div>
                </div>
				...
				<div class="form-group">
					...
					<div class="col-md-4">
					  <input type="text" class="form-control datepicker" name="date" id="date" value="<?php echo date('d-M-Y',strtotime($date_for_update));?>" required>
					</div>
				  </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-4">
                        <input type="text" class="form-control" name="order_number" value="<?php echo $onumber_for_update; ?>" id="order_number" required>
				...
				<div class="form-group">
                    ...
                    <div class="col-md-4">
                        <input type="text" class="form-control" name="order_quantity" value="<?php echo $quantity_for_update; ?>" id="order_quantity" required>
				...
				<div class="form-group">
                    ...
                    <div class="col-md-4">
                       <textarea name="comments" class="input-xlarge" id="comments" rows="5" ><?php echo urldecode($comments_for_update); ?></textarea>
                    </div>
                </div>
                        ...
                        <div class="col-md-6">
                            <div class="row">
                                <div class="col-md-offset-6 col-md-6">
                                    <button type="submit" class="btn default yellow-zed">Update Order</button>
				...
				if($this->uri->segment(3)!=''){
						...
						echo '<input type="hidden"  name="order_hidden_id" id="order_hidden_id" value="'.$this->uri->segment(3).'">';
				}
