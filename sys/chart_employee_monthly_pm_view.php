...
$product_color = array('#E43A45','#ffce00','#28ABE3','#E8B71A','#542733','#5A6A62','#C6D5CD','#FDF200','#69D2E7','#F38630','#588C73','#5E412F','#DB3340','#E8B71A','#F7EAC8','#1FDA9A','#28ABE3','#E94C6F','#542733','#5A6A62','#C6D5CD','#FDF200','#69D2E7','#F38630','#588C73','#5E412F');
$pm_status = array('Pending','Pending Verification','Completed');
//print_r($pm_status);
?>
...
$(document).ready(function() {
			...
			foreach ($pm_status AS $status) {
				...
				for ($i=0;$i<$months_to_show;$i++) { // Loops through months
					...
					$cquery = $this->db->query("SELECT `tbl_complaints`.*,`user`.first_name,`tbl_cities`.city_name,`tbl_clients`.client_name,`tbl_products`.product_name,`tbl_instruments`.serial_no,`tbl_area`.area
						...
						WHERE assign_to = $fk_employee_id AND `tbl_complaints`.complaint_nature='PM' AND `tbl_complaints`.status = '".$status."' AND CAST(`date` AS DATE) BETWEEN '".$first_of_month."' AND '".$last_of_month."' ORDER BY `date` ASC");
						...
						foreach ($cresult AS $pm) {
							...
							$string .= "<br/>".$pm['product_name']." - ".$pm['client_name']." - ".$time_taken;
						}
    ...
    $('#chart_employee_monthly_pm').highcharts(options);
	});
</script>
<script>
...
<div class="portlet solid bordered light bg-inverse">
            ...
            <div class="portlet-body">
					<?php  if( $this->session->flashdata('note') == 'PM Calls List')
						echo '<div class="alert alert-success">PM comments updated successfully!</div>';
			?>
			<div class="row">
                    <div class="col-md-12"> 
...
<?php 
	$complaint_nature = "PM";
									  ...
									  WHERE tbl_complaints.complaint_nature='PM' AND CAST(tbl_complaints.`date` AS DATE) BETWEEN '$start_date' AND '$end_date' AND assign_to = '$fk_employee_id'
									  
									  order by `pk_complaint_id` DESC");
           							  $dbresResult=$dbres->result_array();
									  $pm_missing_comments = 0;
									  if (sizeof($dbresResult) == "0") {
										  ...
										  foreach ($dbresResult as $complaint_list) {
											...
											if (trim($complaint_list['report_comments']) == "") $pm_missing_comments++;
											...
											<tr class=" 
											  ...
											  ">
												...
												<a class="btn btn-sm default blue-stripe" 
													  href="<?php  echo base_url().'sys/pm_form/'.$complaint_list["pk_complaint_id"]; ?>">
														PM Form 
														<i class="fa fa-eye"></i>
													  </a>
...
?>
					  ...
					  <?php if(sizeof($dbresResult)>0) { ?>
						<div class="col-md-offset-5 col-md-7">
							...
							<input type="hidden" name = "div" value = "dpm" />
							<input type="hidden" name = "portlet" value = "PM Calls List" />
 ...
 $query="insert INTO `tbl_report_errors_temp` SET 	
				  ...
				  `month`							='".$end_date."',
				  `pm_missing_comments`				='".$pm_missing_comments."'
