...
<?php if ($this->session->userdata('userrole')=='Admin' || $this->session->userdata('userrole')=='secratery'){ ?>		
		...
		<div class="portlet light bg-inverse" >
						...
						<div class="portlet-body">
						        <div class="row">
                            	<form method="get" action="<?php echo base_url();?>sys/chart_sap_territory_visit_review">
                                <div class="col-md-4">
...
$project_statuses = array('Active','Never Visited','Passive','Ignored','Neglected');


$categories = array('Tender','IDx','CIF','CDx','HDx','MDx','Manual Kits');
$category_ids = array('1','2','3','4','5','6','7');

$project_types = array('Active','Never Visited','Passive','Ignored','Neglected');
//echo "unnus";
//print_r($product_ids);
...
$(document).ready(function() {
	...
	var options = {
        ...
        xAxis: {
			...
			categories: ['Active','Never Visited','Passive','Ignored','Neglected']
        },
						 ...
						 $pc = $project['client_name'] . "- (" . $ty->num_rows().")";
						  if($ty->num_rows()>0) { // IF at least one project
							 ...
							 // if (sizeof($rvr)>0 && $sap!=0) $your_date = strtotime($rvr[0]["recent_visit_date"]);
							  ...
							  elseif ($project_status == "Active" && $project_type == "Active" && $mydiffrence<=30) {
								  $active_projects++;
								  $s_active .= $pc.",<br/>";
							  }
							 /* else {
