                          ...
                          <div class="col-md-4">
						   ...
						   $qua="SELECT * from user where fk_office_id =  '". $rt21[0]['fk_office_id'] ."' AND userrole IN('FSE','Salesman','Supervisor') ORDER BY  `fk_office_id` ,  `userrole` ASC ";
							...
							foreach($rta as $valuea)
							{
								...
								$qu="select * from tbl_complaints where assign_to='".$userid."' AND complaint_nature='PM' AND status!='Pending'";
								$gh=$this->db->query($qu);
								$result = $gh->result_array();
								$pending_pm = sizeof($result);

								$qu="select * from tbl_complaints where assign_to='".$userid."' AND complaint_nature='complaint' AND `date` BETWEEN NOW() - INTERVAL 30 DAY AND NOW()";
								$gh=$this->db->query($qu);
								$result = $gh->result_array();
								...
								$qu="select * from tbl_complaints where assign_to='".$userid."' AND complaint_nature='PM' AND `date` BETWEEN NOW() - INTERVAL 30 DAY AND NOW()";
								$gh=$this->db->query($qu);
								$result = $gh->result_array();
								$month_pm = sizeof($result);
								//echo "Pending TS: ".$pending_ts;

								echo "<p>".$valuea['first_name']." <span class='label bg-red'>Pending TS: ".$pending_ts."</span>, <span class='label bg-yellow'>Pending PM: ".$pending_pm."</span>, <span class='label bg-green'>TS (30 Days): ".$month_ts."</span>, <span class='label bg-blue'>PM (30 Days): ".$month_pm."</span></p>";								
							}
						   ?>
						   </div>
                           <script>
