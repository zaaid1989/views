...
$(document).ready(function() {
        ...
        series: [{
			...
			<?php
				$cq = 	$this->db->query("SELECT *
									FROM tbl_complaints
									WHERE complaint_nature='PM' AND assign_to = '$fk_employee_id'");
				...
				foreach ($pr AS $pm) {
					$cq = 	$this->db->query("SELECT * FROM `tbl_complaints`
							LEFT JOIN tbl_instruments ON tbl_instruments.pk_instrument_id = tbl_complaints.fk_instrument_id
							LEFT JOIN tbl_products ON tbl_products.pk_product_id = tbl_instruments.fk_product_id
							WHERE complaint_nature='PM' AND assign_to = '$fk_employee_id' AND pk_product_id = '".$pm['pk_product_id']."' ");
    ...
    $('#chart_pm_equipment_distribution').highcharts(options);
	});
</script>
